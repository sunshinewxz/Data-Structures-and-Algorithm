...
public:
	...
	{
		item_size = is;			//size_t 相关unsigned类型，其大小足以保存存储内存中对象的大小
		min_value = mv;
	}
	~Apriori(){};
	...
	map< vector<string>,unsigned int> apri_gen(unsigned int K, map< vector<string>,unsigned int> K_item);		//连接k-1级频繁项，得到第k级频繁项
	void showAprioriItem(unsigned int K,map< vector<string>,unsigned int> showmap);						//展示频繁项集
private:
...
void Apriori::getItem()								//用户输入最初的事务集
{
	...
	for(int i=0;i<ci;i++)
	{
		...
		{
			temp.push_back(str);					//push_back:在vector后面加一个数据
		}
...
map< vector<string>,unsigned int>Apriori::find_freitem()
{
	...
	{
		vector<string> vec = mit->second;										//.second	->second??????
		if(vec.size()!=0)
			break;
	}
	...
	while(1)
	{
		...
		map< vector<string>,unsigned int> pre_K_item = K_item;					//判断是否需要进行下一次查找
		size_t Kitemsize = K_item.size();
		if(Kitemsize != 1 && i != 1)											//存储应该删除的第K级频繁项集，不能和其他K级频繁项集构成第K+1级项集的集合
		{
